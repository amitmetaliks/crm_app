		...
		<div class="mx-auto max-w-xl space-y-4 p-4">
			...
			<template v-else>
				...
				<div v-if="c.claimed" class="aa-card text-center">
					...
					<p class="mt-2 font-semibold text-navy-700 dark:text-white">{{ $t("Conveyance already claimed") }}</p>
					<p class="mt-1 text-sm text-gray-500">
						{{ inr(c.claim?.amount) }} · {{ c.claim?.approval_status || c.claim?.status }}
					</p>
					<router-link to="/expense" class="mt-3 inline-block text-sm font-semibold text-saffron">{{ $t("View my claims") }}</router-link>
				</div>

				<!-- No rate configured -->
				<EmptyState
					v-else-if="c.rate_missing"
					:title='$t("No travel rate set")'
					:subtitle='$t("Ask HR to set &#39;Travel Allowance Per Km&#39; on your Sales Person record, then this will calculate automatically.")'
				/>

				<template v-else>
					<!-- The calculation -->
					<div class="aa-card">
						<p class="text-xs uppercase tracking-wide text-gray-400">{{ $t("Distance recorded today") }}</p>
					...
					<div class="aa-card">
						...
						<div v-if="correct" class="mt-3 space-y-3">
							<div>
								<label class="text-xs text-gray-500">{{ $t("Actual distance (km)") }}</label>
								<input v-model.number="km" type="number" min="0" step="0.1" class="aa-input w-full" placeholder="e.g. 82" />
							</div>
							<div>
								<label class="text-xs text-gray-500">{{ $t("Reason") }} <span class="text-red-500">*</span></label>
							...
							<div>
								<label class="text-xs text-gray-500">{{ $t("Attach proof (optional)") }}</label>
							...
							<p v-if="km" class="rounded-lg bg-amber-50 p-2 text-center text-sm text-amber-800"> {{ $t("Claiming") }} <strong>{{ km }} km</strong> = <strong>{{ inr(km * c.rate_per_km) }}</strong>
